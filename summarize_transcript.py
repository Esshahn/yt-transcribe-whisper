...
def main():
    ...
    for transcript_file in os.listdir(transcripts_dir):
        if transcript_file.endswith('.txt') and not transcript_file.endswith('_summary.txt'):
            ...
            video_id = transcript_file.replace('trimmed_', '').split('.')[0]
            video_url = f"https://www.youtube.com/watch?v={video_id}"
            
            print(f"Creating summary for {transcript_path}")
            summary_path = create_summary(transcript_path, video_url)
            
            # Write to pending_posts.txt for slack_poster.py
            with open('pending_posts.txt', 'a', encoding='utf-8') as f:
                f.write(f"{summary_path}|{video_url}\n")
